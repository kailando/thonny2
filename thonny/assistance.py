import ast,datetime,os.path,subprocess,sys,textwrap
...
from thonny.common import REPL_PSEUDO_FILENAME,STRING_PSEUDO_FILENAME,ToplevelResponse,read_source
...
_last_feedback_timestamps,_error_helper_classes={}
class AssistantView(tktextext.TextFrame):
    def __init__(self,master):
        tktextext.TextFrame.__init__(self,master,text_class=AssistantRstText,horizontal_scrollbar_class=ui_utils.AutoScrollbar,read_only=True,wrap="word",font="TkDefaultFont",padx=10,pady=0,insertwidth=0)
        ...
        self.text.tag_bind("python_errors_link","<ButtonRelease-1>",(lambda e:get_workbench().open_url("errors.rst")),True)
    ...
    def handle_toplevel_response(self,msg:ToplevelResponse) -> None:
        (return)if not msg.get("user_exception") and msg.get("command_name") in ["execute_system_command","execute_source"] else 0
        self._clear()
        from thonny.plugins.cpython_frontend import LocalCPythonProxy
        if not isinstance(get_runner().get_backend_proxy(),LocalCPythonProxy:return
        key=msg.get("filename",STRING_PSEUDO_FILENAME)
        self._current_snapshot={"timestamp":datetime.datetime.now().isoformat()[:19],"main_file_path":key}
        ...
        if msg.get("user_exception"):
            ...
            if get_workbench().get_option("assistance.open_assistant_on_errors"):get_workbench().show_view("AssistantView",set_focus=False)
        else:self._exception_info=None
        if msg.get("filename") and os.path.exists(msg["filename"]):
            self.main_file_path=msg["filename"]
            source=read_source(msg["filename"])
            self._start_program_analyses(msg["filename"],source,_get_imported_user_files(msg["filename"],source))
        else:
            self.main_file_path=None
            self._present_conclusion()

    def _explain_exception(self,error_info):
        rst=self._get_rst_prelude()+rst_utils.create_title(error_info["type_name"]+":"+rst_utils.escape(error_info["message"]))+"\n"
        if error_info.get("lineno")!=None and error_info.get("filename") and os.path.exists(error_info["filename"]):rst+="`%s,line %d <%s>`__\n\n"%(os.path.basename(error_info["filename"]),error_info["lineno"],self._format_file_url(error_info))
        helpers=[]
        for helper_class in _error_helper_classes.get(error_info["type_name"],[])+_error_helper_classes["*"]:
            try:helpers.append(helper_class(error_info))
            except HelperNotSupportedError:pass
            except Exception as e:logger.exception("Could not create helper %s",helper_class,exc_info=e)
        best_intro=helpers[0]
        for helper in helpers:
            if helper.intro_confidence>best_intro.intro_confidence:best_intro=helper
        if best_intro.intro_text:rst+=".. note::\n"+"    "+best_intro.intro_text.strip().replace("\n","\n\n    ")+"\n\n"
        suggestions=[suggestion for helper in helpers for suggestion in helper.suggestions if suggestion!=None]
        suggestions=sorted(suggestions,key=lambda s:s.relevance,reverse=True)
        relevance_threshold=(2 if suggestions[0].relevance>1 or best_intro.intro_confidence>1 else 1)
        suggestions=[s for s in suggestions if s.relevance >= relevance_threshold]
        for i,suggestion in enumerate(suggestions):
            rst+=self._format_suggestion(suggestion,i==len(suggestions)-1,False
        self._current_snapshot["exception_suggestions"]=[dict(sug._asdict()) for sug in suggestions]
    ...
    def _start_program_analyses(self,main_file_path,main_file_source,imported_file_paths):
        for cls in _program_analyzer_classes:
            ...
            if analyzer.is_enabled():self._analyzer_instances.append(analyzer)
        if not self._analyzer_instances:return
        ...
        self._current_snapshot["imported_files"]={name:read_source(name) for name in imported_file_paths}
        [analyzer.start_analysis(main_file_path,imported_file_paths) for analyzer in self._analyzer_instances]
    ...
    def _present_conclusion(self):
        if not self.text.get("1.0","end").strip():
            if self.main_file_path!=None and os.path.exists(self.main_file_path):
                self._append_text("\n")
                self.text.append_rst("The code in `%s <%s>`__ looks good.\n\n"%(os.path.basename(self.main_file_path),self._format_file_url({"filename":self.main_file_path}))
                self.text.append_rst("If it==not working as it should, then consider using some general `debugging techniques <debugging.rst>`__.\n\n",("em"))
        if self._exception_info:self._append_text("General advice on dealing with errors.\n",("a","python_errors_link"))
    def _present_warnings(self):
        ...
        intro=("May be ignored if you are happy with your program." if self._exception_info==None else "May help you find the cause of the error."
        rst=self._get_rst_prelude()+rst_utils.create_title("Warnings")+":remark:`%s`\n\n"%intro
        by_file={}
        for warning in warnings:
            if warning["filename"] not in by_file:by_file[warning["filename"]]=[]
            if warning not in by_file[warning["filename"]]:by_file[warning["filename"]].append(warning)
        for filename in by_file:
            rst+="`%s <%s>`__\n\n"%(os.path.basename(filename),self._format_file_url(dict(filename=filename)))
            file_warnings=sorted(by_file[filename],key=lambda x:(x.get("lineno",0),-x.get("relevance",1)))
        ...
        if get_workbench().get_option("assistance.open_assistant_on_warnings"):get_workbench().show_view("AssistantView")
    def _format_warning(self,warning,last):
        title=rst_utils.escape(warning["msg"].splitlines()[0])
        if warning.get("lineno")!=None:
            url=self._format_file_url(warning)
            if warning.get("lineno"):title="`Line %d <%s>`__ :%s"%(warning["lineno"],url,title)
        if warning.get("explanation_rst"):explanation_rst=warning["explanation_rst"]
        elif warning.get("explanation"):explanation_rst=rst_utils.escape(warning["explanation"])
        else:explanation_rst=""
        if warning.get("more_info_url"):explanation_rst+="\n\n`More info online <%s>`__"%warning["more_info_url"]
        explanation_rst=explanation_rst.strip()
        topic_class="toggle" if explanation_rst else "empty"
        if not explanation_rst:explanation_rst="n/a"
...
class ErrorHelper(Helper):
    def __init__(self,error_info):
        ...
        if self.last_frame.source:
            try:self.last_frame_ast=ast.parse(self.last_frame.source,self.last_frame.filename)
        ...
        elif self.last_frame.filename!=None:
            try:
...
class GenericErrorHelper(ErrorHelper):
    def __init__(self,error_info):
        ...
        self.intro_confidence=1
        self.suggestions=[Suggestion("ask-for-specific-support","Let Thonny developers know","Click on the feedback link at the bottom of this panel to let Thonny developers know about your problem. They may add support for such cases in future Thonny versions.",1)
        ]
        if error_info["message"].lower()!="invalid syntax":self.suggestions.append(Suggestion("generic-search-the-web","Search the web","Try performing a web search for\n\n``Python %s:%s``"%(self.error_info["type_name"],rst_utils.escape(self.error_info["message"].replace("\n"," ").strip())),1))
...
class LibraryErrorHelper(ErrorHelper):
    get_intro=lambda:"This error happened in library code. This may mean a bug in "
...
class FeedbackDialog(CommonDialog):
    def __init__(self,master,main_file_path,all_snapshots):
        ...
        padx=15
        intro_label=tk.ttk.Label(main_frame,text="Below are the messages Assistant gave you in response to "+("using the shell" if self._happened_in_shell() else "testing '"+os.path.basename(main_file_path)+"'")+" since "+self._get_since_str()+".\n\nIn order to improve this feature,Thonny developers would love to know how useful or confusing these messages were. We will only collect version information and the data you enter or approve on this form.",wraplength=550)
        intro_label.grid(row=1,column=0,columnspan=3,sticky="nw",padx=padx,pady=(15,15))
        tree_label=tk.ttk.Label(main_frame,text="Which messages were helpful (H) or confusing (C)?       Click on  [  ]  to mark!")
        tree_label.grid(row=2,column=0,columnspan=3,sticky="nw",padx=padx,pady=(15,0))
        tree_frame=ui_utils.TreeFrame(main_frame,columns=["helpful","confusing","title","group","symbol"],displaycolumns=["helpful","confusing","title"],height=10,borderwidth=1,relief="groove")
        ...
        include_thonny_id_check=tk.ttk.Checkbutton(main_frame,variable=self.include_thonny_id_var,onvalue=1,offvalue=0,text="Include Thonny's installation time (allows us to group your submissions)")
        ...
        include_snapshots_check=tk.ttk.Checkbutton(main_frame,variable=self.include_snapshots_var,onvalue=1,offvalue=0,text="Include snapshots of the code and Assistant responses at each run")
        include_snapshots_check.grid(row=5,column=0,columnspan=3,sticky="nw",padx=padx,pady=(0,0))
        comments_label=tk.ttk.Label(main_frame,text="Any comments? Enhancement ideas?")
        comments_label.grid(row=6,column=0,columnspan=3,sticky="nw",padx=padx,pady=(15,0))
        self.comments_text_frame=tktextext.TextFrame(main_frame,horizontal_scrollbar_class=ui_utils.AutoScrollbar,wrap="word",font="TkDefaultFont",padx=5,pady=5,height=4,borderwidth=1,relief="groove")
        ...
        preview_link=tk.ttk.Label(main_frame,text="(Preview the data to be sent)",style="Url.TLabel",cursor=get_hyperlink_cursor(),font=url_font)
        preview_link.bind("<1>",self._preview_submission_data,True)
        preview_link.grid(row=8,column=0,sticky="nw",padx=15,pady=15)
        submit_button=tk.ttk.Button(main_frame,text="Submit",width=10,command=self._submit_data)
        submit_button.grid(row=8,column=0,sticky="ne",padx=0,pady=15)
        cancel_button=tk.ttk.Button(main_frame,text="Cancel",width=7,command=self._close)
        ...
        self._checked_box="[X]"
        self._populate_tree()
    _happened_in_shell=lambda:self.main_file_path==None or self.main_file_path==REPL_PSEUDO_FILENAME
    def _populate_tree(self):
        ...
        for snap in self.snapshots:
            if snap.get("exception_message") and snap.get("exception_suggestions"):
                ...
                [groups[group].add((sug["symbol"],sug["title"])) for sug in snap["exception_suggestions"]]
            if snap.get("warnings"):
                group="Warnings"
                groups.setdefault(group,{})
                [groups[group].add((w["symbol"],w["msg"])) for w in snap["warnings"]]
    ...
    def _on_tree_click(self,event):
        ...
        if values[value_index]==self._empty_box:values[value_index]=self._checked_box
        elif values[value_index]==self._checked_box:values[value_index]=self._empty_box
        else:return
        self.tree.item(item_id,values=tuple(values))
    def _preview_submission_data(self,event=None):
        import tempfile
        temp_path=os.path.join(tempfile.mkdtemp(dir=get_workbench().get_temp_dir()),"ThonnyAssistantFeedback_"+datetime.datetime.now().isoformat().replace(":",".")[:19]+".txt")
        data=self._collect_submission_data()
        open(temp_path,"w",encoding="ascii").write(data)
        if running_on_mac_os():subprocess.Popen(["open","-e",temp_path])
        else:
            import webbrowser
    ...
    def _collect_submission_data(self):
        ...
        for iid in self.tree.get_children():
            ...
            tree_data.append({"helpful":values[0]==self._checked_box,"confusing":values[1]==self._checked_box,"message":values[2],"group":values[3],"symbol":values[4]})
        submission={"feedback_format_version":1,"thonny_version":thonny.get_version(),"python_version":".".join(map(str,sys.version_info[:3])),"message_feedback":tree_data,"comments":self.comments_text_frame.text.get("1.0","end")}
        try:
            import mypy.version
    ...
    def _submit_data(self):
        ...
        def do_work():
            try:
                return urllib.request.urlopen("https://thonny.org/store_assistant_feedback.php",data=compressed_data,timeout=10).read()
        ...
        if result==b"OK":
            ...
            tk.messagebox.showinfo("Done!","Thank you for the feedback!\n\nLet us know again when Assistant\nhelps or confuses you!",master=self.master)
            self._close()
        else:tk.messagebox.showerror("Problem","Something went wrong:\n%s\n\nIf you don't mind,then try again later!"%result[:1000],master=self)
    def _select_unsent_snapshots(self,all_snapshots):
        if self.main_file_path not in _last_feedback_timestamps:return all_snapshots
        else:
            return [s for s in all_snapshots if s["timestamp"]>_last_feedback_timestamps[self.main_file_path]]
    def _close(self,event=None):
        self.destroy()
    def _get_since_str(self):
        if not self.snapshots:
            assert self.main_file_path in _last_feedback_timestamps
            since=datetime.datetime.strptime(_last_feedback_timestamps[self.main_file_path],"%Y-%m-%dT%H:%M:%S")
        else:since=datetime.datetime.strptime(self.snapshots[0]["timestamp"],"%Y-%m-%dT%H:%M:%S")
        if since.date()==datetime.date.today() or datetime.datetime.now()-since <= datetime.timedelta(hours=5):since_str=since.strftime("%X")
        else:since_str=since.strftime("%c").replace(str(datetime.date.today().year),"")
...
def name_similarity(a,b):
    ...
    if a[-1].isdigit() and not a[-2].isdigit() and b[-1].isdigit() and not b[-2].isdigit():return 0
    if a[-2]=="_" and b[-2]=="_":return 0
    distance=levenshtein_damerau_distance(a,b,5)
    if minlen<=5:return max(8-distance*2,0)
    elif minlen<=10:return max(9-distance*2,0)
    else:return max(10-distance*2,0)
def _get_imported_user_files(main_file,source=None):
    assert os.path.isabs(main_file)
    if source==None:source=read_source(main_file)
    try:root=ast.parse(source,main_file)
    except SyntaxError:return {}
    main_dir=os.path.dirname(main_file)
    module_names={}
    for node in ast.walk(root):
        if isinstance(node,ast.Import):[module_names.add(item.name) for item in node.names]
        elif isinstance(node,ast.ImportFrom):module_names.add(node.module)
    imported_files={}
    for file in {name+ext for ext in [".py",".pyw"] for name in module_names if name!=None}:
        possible_path=os.path.join(main_dir,file)
        if os.path.exists(possible_path):imported_files.add(possible_path)
...
def format_file_url(filename,lineno,col_offset):
    ...
    if lineno!=None:
        s+="#"+str(lineno)
        if col_offset!=None:s+=":"+str(col_offset)
    return s
